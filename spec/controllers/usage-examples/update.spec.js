...
describe('POST /usage_examples/:id', function() {
    ...
    describe('failure', function() {
        it(`should respond with 401 Unauthorized if the request does not contain an 'authorization' header`, function() {
            return request(server).post(`/usage_examples/${usageExample.get(k.Attr.Id)}`).send(validBody).expect(401);
        });

        it('should respond with 400 Bad Request if the :id parameter is not a number', function() {
            return request(server).post(`/usage_examples/not_a_number`).set(k.Header.Authorization, authorization).send(validBody).expect(400);
        });

        it('should respond with 400 Bad Request if the :id parameter is 0', function() {
            return request(server).post(`/usage_examples/0`).set(k.Header.Authorization, authorization).send(validBody).expect(400);
        });
    ...
    describe('success', function() {
        it('should respond with an X-GN-Auth-Token header', async function() {
            const response = await request(server).post(`/usage_examples/${usageExample.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send(validBody);
        ...
        it('should update the "text" property of the UsageExample record', async function() {
            await request(server).post(`/usage_examples/${usageExample.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send(validBody);
