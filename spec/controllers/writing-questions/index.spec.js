...
describe('GET /videos/:id/writing_questions', function() {
    ...
    describe('failure', function() {
        it(`should respond with 400 Bad Request if the :id param is not a number`, function(done) {
            request(server).get(`/videos/not_a_number/writing_questions`).set('authorization', authorization).expect(400, done);
        });

        it(`should respond with 400 Bad Request if the :id param is 0`, function(done) {
            request(server).get(`/videos/0/writing_questions`).set('authorization', authorization).expect(400, done);
        });

        it(`should respond with 400 Bad Request if the 'count' value is not a number`, function(done) {
            request(server).get(`/videos/${englishVideoId}/writing_questions?count=not_a_number`).set('authorization', authorization)
                .expect(400, done);
        });

        it(`should respond with 400 Bad Request if the 'count' value is 0`, function(done) {
            request(server).get(`/videos/${englishVideoId}/writing_questions?count=0`).set('authorization', authorization).expect(400, done);
        });

        it('should respond with 404 Not Found if no WritingQuestion records can be found for the video id', function(done) {
            request(server).get(`/videos/99999999/writing_questions`).set('authorization', authorization).expect(404, done);
        });
    });

    describe('success', function() {
        it('should respond with an X-GN-Auth-Token header', function() {
            return request(server).get(`/videos/${englishVideoId}/writing_questions`).set('authorization', authorization).then(function(response) {
        ...
        it(`should respond with 200 OK for a successful request`, function(done) {
            request(server).get(`/videos/${englishVideoId}/writing_questions`).set('authorization', authorization).expect(200, done);
        });
        ...
        it(`should return all the writing_questions records for the specified video subcategory`, async function() {
            const response = await request(server).get(`/videos/${englishVideoId}/writing_questions`).set('authorization', authorization);
        ...
        it(`should return as many or fewer writing answer records as specified by the 'count' query parameter`, function() {
            return request(server).get(`/videos/${englishVideoId}/writing_questions?count=2`).set('authorization', authorization).then(function(response) {
