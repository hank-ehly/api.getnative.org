...
describe('PATCH /user', function() {
    ...
    beforeEach(function() {
        ...
        return SpecUtil.login().then(function(_) {
            ...
            return db.User.update(context, {where: {id: user.id}});
        });
    });
    ...
    describe('response.headers', function() {
        it('should respond with an X-GN-Auth-Token header', function() {
            return request(server).patch('/user').set('authorization', authorization).send(validBody).then(function(response) {
    ...
    describe('response.success', function() {
        it(`should return 204 No Content for a valid request`, function(done) {
            request(server).patch('/user').set('authorization', authorization).send(validBody).expect(204, done);
        });

        it(`should respond with 304 Not Modified if the request can authenticate but contains no body`, function(done) {
            request(server).patch('/user').set('authorization', authorization).expect(304, done);
        });
    ...
    describe('response.failure', function() {
        it(`should respond with 401 Unauthorized if the request does not contain an 'authorization' header`, function(done) {
            request(server).patch('/user').send(validBody).expect(401, done);
        });

        it(`should return 400 Bad Request if the email_notifications_enabled request parameter is not a boolean`, function(done) {
            request(server).patch('/user').set('authorization', authorization).send({
    ...
    describe('other', function() {
        it(`should change the users' email_notifications_enabled setting`, function() {
            return request(server).patch('/user').set('authorization', authorization).send({email_notifications_enabled: true}).then(function() {
        ...
        it(`should change the users' browser_notifications_enabled setting`, function() {
            return request(server).patch('/user').send({browser_notifications_enabled: true}).set('authorization', authorization).then(function() {
                return db.User.findById(user.id).then(function(_user) {
        ...
        it(`should change the users' default_study_language_code setting`, function() {
            return request(server).patch('/user').set('authorization', authorization).send({default_study_language_code: 'ja'}).then(function() {
                return db.User.findById(user.id).then(function(_user) {
                    assert.equal(_user.default_study_language_code, 'ja');
                });
            });
        });
    });
