...
describe('POST /collocation_occurrences/:id', function() {
    ...
    describe('failure', function() {
        it(`should respond with 401 Unauthorized if the request does not contain an 'authorization' header`, function() {
            return request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).send(validBody).expect(401);
        });

        it('should respond with 400 Bad Request if the :id parameter is not a number', function() {
            return request(server).post(`/collocation_occurrences/not_a_number`).set(k.Header.Authorization, authorization).send(validBody).expect(400);
        });

        it('should respond with 400 Bad Request if the :id parameter is 0', function() {
            return request(server).post(`/collocation_occurrences/0`).set(k.Header.Authorization, authorization).send(validBody).expect(400);
        });

        it('should respond with 404 Not Found if no CollocationOccurrence record for the provided :id exists', function() {
            return request(server).post(`/collocation_occurrences/${Math.pow(10, 6)}`).set(k.Header.Authorization, authorization).send(validBody).expect(404);
        });

        it('should respond with 400 Bad Request if the "ipa_spelling" parameter is an empty string', function() {
            return request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send({ipa_spelling: _.stubString()}).expect(400);
        });

        it('should respond with 400 Bad Request if the "ipa_spelling" parameter is not a string', function() {
            return request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send({ipa_spelling: _.stubObject()}).expect(400);
        });
        ...
        it('should respond with 400 Bad Request if the "ipa_spelling" parameter is longer than 100 chars', function() {
            ...
            return request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send({ipa_spelling: longString}).expect(400);
        });

        it('should respond with 304 if the request body is empty', function() {
            return request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send({}).expect(304);
        });
    });

    describe('success', function() {
        it('should respond with an X-GN-Auth-Token header', async function() {
            const response = await request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send(validBody);
        ...
        it('should return a 204 response for a valid request', function() {
            return request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send(validBody).expect(204);
        });

        it('should update the ipa_spelling property of the CollocationOccurrence record', async function() {
            await request(server).post(`/collocation_occurrences/${collocation.get(k.Attr.Id)}`).set(k.Header.Authorization, authorization).send(validBody);
